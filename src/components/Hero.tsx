...
export function Hero({ videoUrl }: HeroProps) {
  ...
  return (
    <section className="hero-always-dark relative h-screen min-h-[620px] w-full overflow-hidden flex items-center">
      {/* Background media */}
      {videoUrl ? (
        <video
      ...
      <Container className="relative z-10 h-full flex flex-col justify-center text-center animate-fade-up">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link
              href="/daftar"
              className="hero-btn-primary btn-primary px-10 py-4 text-base md:text-lg font-semibold shadow-xl hover:shadow-2xl hover:scale-[1.02] transition-all"
            >
              Daftar Sekarang
            </Link>
            <Link
              href="/gallery"
              className="hero-btn-outline btn-outline text-white border-white/40 hover:border-white/70 hover:bg-white/10 backdrop-blur-sm px-10 py-4 text-base md:text-lg font-semibold shadow-lg hover:shadow-xl hover:scale-[1.02]"
            >
              Lihat Galeri
            </Link>
          </div>
